...
def detailGetDateList(start_date, end_date, conf):
    start_y, start_m, start_d = dateParser(start_date)
    end_y, end_m, end_d = dateParser(end_date)
    ...
    while dateEqual(current_y, current_m, current_d, end_y, end_m, end_d) == False:
        ...
        if data_dict.has_key(date_int):
            date_unit_list.append(data_dict[date_int])
        else:
            date_unit_list.append([])
...
def getCodeSequenceUnitList(start_date, end_date, conf):
    ...
    # aggregate 4 QuadClass of each time unit
...
def detailGetCodeSequenceUnitList(start_date, end_date, conf):
    date_unit_list = detailGetDateList(start_date, end_date, conf)
    time_unit_list = getTimeUnitList(date_unit_list, conf)
    code_time_unit_list = []

    # aggregate EventRootCodes of each time unit

    for idx in range(len(time_unit_list)):
        code_count = {}
        for root_code in range(16, 20):
            code_count[root_code] = 0
        for event in time_unit_list[idx]:
            if check(event):
                event_code = event['EventRootCode']
                if code_count.has_key(event_code):
                    code_count[event_code] += 1
        code_time_unit_list.append(code_count.values())
    ...
    for idx in range(len(code_time_unit_list) - conf['sequence_len'] + 1):
        # code_sequence_unit: [sequence_len, 5]
        code_sequence_unit = []
        for bias in range(conf['sequence_len']):
            code_sequence_unit.append(code_time_unit_list[idx + bias])
        code_sequence_unit_list.append(code_sequence_unit)
...
def detailCheckAndGet(rebuild, tag, conf, start_date, end_date):
    prepared_filename = conf['origin_path'] + 'pickledData/' + 'pickled_detail_' + \
                        conf['entity_pair'] + '_' + \
                        tag + '_' + \
                        str(conf['moving_average'][0]) + '_' + \
                        str(conf['moving_average'][1]) + '_' + \
    ...
    if os.path.isfile(prepared_filename) and rebuild == False:
        print 'Already have detailed data {}'.format(prepared_filename)
        with open(prepared_filename) as f:
            code_sequence_detail_list = pickle.load(f)
    else:
        code_sequence_detail_list = detailGetCodeSequenceUnitList(start_date, end_date, conf)
        with open(prepared_filename, 'w') as f:
            pickle.dump(code_sequence_detail_list, f)
            print 'Pickled {}'.format(prepared_filename)

    return code_sequence_detail_list
...
def getMultiDataLoader(tag, conf, shuffle=True):
    ...
    code_sequence_unit_array.append(detailCheckAndGet(True, tag, conf, start_date, end_date))
