...
class EMILSTMTrainer(nn.Module):
    def __init__(self, conf):
        super(EMILSTMTrainer, self).__init__()
        self.conf = conf

        self.model = EMILSTM(conf['device'], conf['topK'], conf['input_size'], conf['hidden_size'], conf['num_layers'], conf['num_output'], conf['drop_out'])
...
class EMILSTMChain(nn.Module):
    def __init__(self, device, topK, input_size, hidden_size, num_layers, bias = True):
        super(EMILSTMChain, self).__init__()
        self.device = device
        self.num_layers = num_layers
        self.topK = topK

        self.cell = ELSTMCell(device, input_size, hidden_size, bias)
...
class EMILSTMCell(nn.Module):
    def __init__(self, device, input_size, hidden_size, bias = True):
        super(EMILSTMCell, self).__init__()
        self.device = device
