...
def getCodeSequenceUnitList(start_date, end_date, conf):
    ...
    for idx in range(len(time_unit_list)):
        ...
        for root_code in range(16, 21):
            code_count[root_code] = 0
        for event in time_unit_list[idx]:
            if check(event):
                event_code = event['QuadClass']
                code_count[event_code] += 1
                root_code = event['EventRootCode']
                if root_code >= 16 and root_code < 21:
                    code_count[root_code] += 1
    ...
    for idx in range(len(code_time_unit_list) - conf['sequence_len'] + 1):
        # code_sequence_unit: [sequence_len, 4 + 5]
        code_sequence_unit = []
...
def checkAndGet(tag, conf, start_date, end_date):
    prepared_filename = conf['origin_path'] + 'pickledData/' + 'pickled_enhanced_' + \
                        conf['entity_pair'] + '_' + \
                        tag + '_' + \
...
def getDataLoader(tag, conf, shuffle = True):
    ...
    data_set = codeSequenceDataset(code_sequence_unit_list)

    return DataLoader(data_set, batch_size = conf[tag + '_batch_size'], num_workers = 4, shuffle = shuffle, drop_last = True)
